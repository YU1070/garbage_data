...
def transparentOverlay(path):
    ...
    target = cv.resize(bgImg, (300, 300))
    ...
    for i in range(0, 10):
        ...
        readimg = cv.resize(readimg, (100, 100))
        overlay = random_augment(readimg)
        h, w = overlay.shape[:2]
        x = np.random.randint(0, 300 - h)
        y = np.random.randint(0, 300 - w)
...
def generateImage():
    ...
    with open("./ImageSets/train.txt", "w") as wf:
        for i in range(0, 200):
            ...
            annotation = ("./JPEGImages/" + str(i) + ".jpg" + label)
            wf.write(annotation + "\n")
            wf.flush()
